...
std::vector<std::string> HostClient::host_addresses_for_find() {
...
std::string HostClient::external_address_for_mdns(const std::string& address) {
...
void HostClient::find_hosts(ServerCallback<std::vector<Host>>& callback) {
        ...
        while (!isCancelled()) {
            ...
            if (records == 0) {
                ...
                if (status == GS_OK) {
                    ...
                    host.remoteAddress =
                        HostClient::external_address_for_mdns(host.address);
...
void HostClient::cancel_find_hosts() {
    ++findHostsGeneration;
}
#endif

bool HostClient::can_wake_up_host(const Host& host) {
    return WakeOnLanManager::can_wake_up_host(host);
}

void HostClient::wake_up_host(const Host& host,
    ...
    brls::async([host, callback] {
        ...
        for (int attempt = 0; attempt < WAKE_POLL_ATTEMPTS; attempt++) {
            if (connect_to_addresses_sync(host.connection_addresses(),
                                          ...
                                          error)) {
                auto& client = HostClient::instance();
...
void HostClient::cache_server_data(const std::string& address,
...
void HostClient::connect_to_addresses(
...
void HostClient::connect(const std::string& address,
                               ServerCallback<SERVER_DATA>& callback) {
    connect_to_addresses({address}, "", callback);
}

std::string HostClient::active_address(const Host& host) const {
...
void HostClient::connect(const Host& host,
                               ServerCallback<SERVER_DATA>& callback) {
    connect_to_addresses(host.connection_addresses(), host_key(host), callback);
}

void HostClient::pair(const std::string& address, const std::string& pin,
                            ServerCallback<bool>& callback) {
...
void HostClient::pair(const Host& host, const std::string& pin,
                            ServerCallback<bool>& callback) {
    pair(active_address(host), pin, callback);
}

void HostClient::applist(const std::string& address,
...
void HostClient::applist(const Host& host,
                               ServerCallback<AppInfoList>& callback) {
    applist(active_address(host), callback);
}

void HostClient::app_boxart(const std::string& address, int app_id,
...
void HostClient::app_boxart(const Host& host, int app_id,
                                  ServerCallback<Data>& callback) {
    app_boxart(active_address(host), app_id, callback);
}

void HostClient::start(const std::string& address,
...
void HostClient::start(const Host& host, STREAM_CONFIGURATION config,
                             int app_id,
                             ServerCallback<STREAM_CONFIGURATION>& callback) {
    start(active_address(host), config, app_id, callback);
}

void HostClient::quit(const std::string& address,
                            ServerCallback<bool>& callback) {
    with_cached_server_data<bool>(
        address, "Firstly call connect() & pair()...", callback,
...
void HostClient::quit(const Host& host,
                            ServerCallback<bool>& callback) {
