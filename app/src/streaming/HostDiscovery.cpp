...
void HostDiscovery::start() {
    if (addresses.empty()) {
        this->addresses =
            HostClient::host_addresses_for_find();
        if (addresses.empty()) {
    ...
    if (paused) {
        paused = false;
        brls::async([] { HostDiscovery::instance().loop(); });
    }
    brls::sync([this] { getHostsUpdateEvent()->fire(hosts); });
}

void HostDiscovery::pause() { paused = true; }

void HostDiscovery::loop() {
    brls::async([this] {
#if defined(PLATFORM_PSV)
...
HostDiscovery::~HostDiscovery() { paused = true; }
