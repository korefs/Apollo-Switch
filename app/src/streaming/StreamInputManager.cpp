...
constexpr short VK_TAB_KEY = 0x09;
constexpr short VK_F4_KEY = 0x73;
...
StreamInputManager::StreamInputManager() {
    ...
    inputManager
        ...
        ->subscribe([this](brls::KeyState state) {
            ...
            int vkKey = StreamInputManager::glfwKeyToVKKey(state.key);
...
void StreamInputManager::handleDesktopMouseScroll(brls::Point scroll) {
...
void StreamInputManager::sendDesktopMouseScroll(brls::Point scroll) {
...
void StreamInputManager::sendRelativeMouseMove(brls::Point offset) {
...
void StreamInputManager::updateTouchScreenPanDelta(
    brls::PanGestureStatus panStatus) {
    this->panStatus = panStatus;
}

void StreamInputManager::handleRumble(unsigned short controller,
...
void StreamInputManager::handleRumbleTriggers(uint16_t controllerNumber, 
...
void StreamInputManager::dropInput() {
    if (inputDropped)
        return;
    ...
    for (int i = BRLS_KBD_KEY_SPACE; i < BrlsKeyboardScancode::BRLS_KBD_KEY_LAST; i++)  {
        short vkKey = StreamInputManager::glfwKeyToVKKey((BrlsKeyboardScancode)i);
...
GamepadState StreamInputManager::getControllerState(int controllerNum,
                                                       bool specialKey) {
    ...
    const auto suppressedButtons =
        handleHostShortcuts(controllerNum, rawController);
    for (auto button : suppressedButtons)
        rawController.buttons[button] = false;
...
std::vector<brls::ControllerButton>
StreamInputManager::handleHostShortcuts(
    int controllerNum, const brls::ControllerState& controller) {
    auto& state = hostShortcutStates[controllerNum];
    const auto now = std::chrono::steady_clock::now();
    ...
    if (state.action >= 0) {
        ...
        for (auto button : state.buttons) {
            anyPressed |= controller.buttons[button];
            allPressed &= controller.buttons[button];
        }
        ...
        if (state.eligible && !state.triggered &&
            now - state.pressedAt >= std::chrono::seconds(state.holdTime)) {
            state.triggered = true;
            sendHostShortcut(state.action);
        }
    ...
    for (auto action : actions) {
        const auto options = Settings::instance().key_combo_options(action);
        ...
        state.holdTime = options.holdTime;
        state.eligible = true;
...
void StreamInputManager::sendHostShortcut(int action) {
    short key = 0;
    switch (static_cast<KeyComboAction>(action)) {
    ...
    LiSendKeyboardEvent(key, KEY_ACTION_DOWN, MODIFIER_ALT);
    LiSendKeyboardEvent(key, KEY_ACTION_UP, MODIFIER_ALT);
}

void StreamInputManager::resetHostShortcuts() {
    for (auto& state : hostShortcutStates)
        state = {};
}

void StreamInputManager::handleControllers(bool specialKey) {
...
void StreamInputManager::handleInput(bool ignoreTouch) {
    inputDropped = false;
...
short StreamInputManager::controllersToMap() {
...
StreamInputManager::mapController(brls::ControllerState controller) {
...
void StreamInputManager::leftMouseClick() {
    LiSendMouseButtonEvent(BUTTON_ACTION_PRESS, BUTTON_MOUSE_LEFT);
    LiSendMouseButtonEvent(BUTTON_ACTION_RELEASE, BUTTON_MOUSE_LEFT);
}

void StreamInputManager::rightMouseClick() {
...
short StreamInputManager::glfwKeyToVKKey(BrlsKeyboardScancode key) {
    ...
    case BRLS_KBD_KEY_SPACE:
        return 0x20;
    default:
        if (key >= BRLS_KBD_KEY_A && key <= BRLS_KBD_KEY_Z)
            return key - BRLS_KBD_KEY_A + 0x41;
        if (key >= BRLS_KBD_KEY_0 && key <= BRLS_KBD_KEY_9)
            return key - BRLS_KBD_KEY_0 + 0x30;
    }
    return 0;
}
