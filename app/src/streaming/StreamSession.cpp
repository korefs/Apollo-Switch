...
int m_video_format;
static StreamSession* m_active_session = nullptr;
static MoonlightSessionDecoderAndRenderProvider* m_provider = nullptr;


StreamSession* StreamSession::activeSession() {
    return m_active_session;
}

void StreamSession::set_provider(
    MoonlightSessionDecoderAndRenderProvider* provider) {
    m_provider = provider;
}

StreamSession::StreamSession(const std::string& address, int app_id) {
    m_address = address;
    m_app_id = app_id;
...
void StreamSession::set_stream_settings(
...
void StreamSession::connection_stage_starting(int stage) {
    brls::Logger::info("StreamSession: Starting: {}", stages[stage]);
}

void StreamSession::connection_stage_complete(int stage) {
    brls::Logger::info("StreamSession: Complete: {}", stages[stage]);
}

void StreamSession::connection_stage_failed(int stage, int error_code) {
    brls::Logger::error("StreamSession: Failed: {} with error code: {}", stages[stage], error_code);
}

void StreamSession::connection_started() {
    brls::Logger::info("StreamSession: Connection started");
    if (!m_active_session)
        return;
...
void StreamSession::connection_terminated(int error_code) {
    brls::Logger::info("StreamSession: Connection terminated with code: {}", error_code);

    if (!m_active_session)
        return;

    if (m_active_session->m_stop_requested) {
        brls::Logger::info("StreamSession: Termination acknowledged after stop request");
    ...
    if (error_code != 0) {
        brls::Logger::info("StreamSession: Reconnection attempt");
        ...
        m_active_session->start_internal([](const GSResult<bool>& result) {
            if (result.isSuccess()) {
                brls::Logger::info("StreamSession: Reconnected");
            } else {
                brls::Logger::info("StreamSession: Reconnection failed");
                if (m_active_session) {
                    m_active_session->m_is_active = false;
                    m_active_session->m_is_terminated = true;
                }
...
void StreamSession::connection_log_message(const char* format, ...) {
    va_list arglist;
...
void StreamSession::connection_rumble(unsigned short controller,
                                         unsigned short lowFreqMotor,
                                         unsigned short highFreqMotor) {
    StreamInputManager::instance().handleRumble(controller, lowFreqMotor,
                                                   highFreqMotor);
}


void StreamSession::connection_rumble_triggers(uint16_t controllerNumber, 
                                                  uint16_t leftTriggerMotor, 
                                                  uint16_t rightTriggerMotor) 
{
    // StreamInputManager::instance().handleRumbleTriggers(controllerNumber, leftTriggerMotor, rightTriggerMotor);                                                
}

void StreamSession::connection_status_update(int connection_status) {
    if (m_active_session) {
...
void StreamSession::connection_set_hdr_mode(bool use_hdr) {
    if (m_active_session) {
...
int StreamSession::video_decoder_setup(int video_format, int width,
    ...
    if (m_active_session && m_active_session->m_video_decoder) {
        ...
        if (result < 0 && (video_format & VIDEO_FORMAT_MASK_AV1) != 0) {
            ...
            brls::Logger::warning(
                "StreamSession: AV1 hardware decoder setup failed on {}; scheduling one fallback attempt",
...
void StreamSession::video_decoder_start() {
    if (m_active_session && m_active_session->m_video_decoder) {
        m_active_session->m_video_decoder->start();
    }
}

void StreamSession::video_decoder_stop() {
    if (m_active_session && m_active_session->m_video_decoder) {
        m_active_session->m_video_decoder->stop();
    }
}

void StreamSession::video_decoder_cleanup() {
    if (m_active_session && m_active_session->m_video_decoder) {
        m_active_session->m_video_decoder->cleanup();
    }
}

int StreamSession::video_decoder_submit_decode_unit(
...
int StreamSession::audio_renderer_init(
...
void StreamSession::audio_renderer_start() {
    if (m_active_session && m_active_session->m_audio_renderer) {
        m_active_session->m_audio_renderer->start();
    }
}

void StreamSession::audio_renderer_stop() {
    if (m_active_session && m_active_session->m_audio_renderer) {
        m_active_session->m_audio_renderer->stop();
    }
}

void StreamSession::audio_renderer_cleanup() {
    if (m_active_session && m_active_session->m_audio_renderer) {
        m_active_session->m_audio_renderer->cleanup();
    }
}

void StreamSession::audio_renderer_decode_and_play_sample(
...
void StreamSession::start(ServerCallback<bool> callback, bool is_sunshine) {
    m_force_disable_av1 = false;
    m_av1_fallback_attempted = false;
    start_internal(std::move(callback), is_sunshine);
}

void StreamSession::start_internal(ServerCallback<bool> callback,
                                      bool is_sunshine) {
    ...
    if (codec != requestedCodec) {
        brls::Logger::info(
            "StreamSession: Requested codec is unavailable; using {}",
    ...
    case AV1:
        if (m_force_disable_av1) {
            ...
            brls::Logger::info(
                "StreamSession: AV1 fallback active; advertising HEVC and H.264");
            break;
        }
        ...
        brls::Logger::info(
            "StreamSession: Prefer AV1 via {}; HEVC and H.264 fallback enabled",
            hardwareVideoCodecSupport().av1Backend);
        break;
    default:
        break;
            ...
            if (result.isSuccess()) {
                ...
                brls::async([this, callback]() mutable {
                    ...
                    if (result != 0 && m_av1_decoder_failed &&
                        ...
                        m_av1_decoder_failed = false;
                        brls::Logger::warning(
                            "StreamSession: Retrying once without AV1 after {} hardware decoder setup failure; fallback formats are HEVC and H.264",
                            hardwareVideoCodecSupport().av1Backend);
                        LiStopConnection();
                ...
                brls::Logger::error(
                    "StreamSession: Failed to start stream: {}",
                    result.error().c_str());
...
void StreamSession::stop(int terminate_app) {
    if (m_stop_requested)
        return;
...
bool StreamSession::fallback_from_av1() {
    ...
    m_force_disable_av1 = true;
    brls::Logger::warning(
        "StreamSession: AV1 hardware frame decode failed on {}; reconnecting once with HEVC and H.264 fallback",
        hardwareVideoCodecSupport().av1Backend);
    restart();
    return true;
}

void StreamSession::draw(NVGcontext* vg, int width, int height) {
